...
module qw {
  export def enu [] {
    cp ~/AppData/Roaming/nushell/ ~/sources/my-synchronization-context --recursive
  }

  export def inu [] {
    cp ~/sources/my-synchronization-context/nushell/ ~/AppData/Roaming --recursive
  }
   
  export def gw [branch, folder] {
    git worktree add -b $branch $folder master
  }

  export def l [amount] {
    git log --pretty=%h»¦«%aN»¦«%s»¦«%aD | lines | split column "»¦«" sha1 committer desc merged_at | first $amount
  }
  ...
  export def n [] {
    hx ~/sources/my-synchronization-context/vaults/dev
  }

  export def npriv [] {
    hx ~/sources/my-synchronization-context/vaults/personal
  }
